...
--merg functions not needed for build

local GathermateID_to_NodeID = {}

for NodeID, GMID in pairs (NodeID_to_GathermateID) do
	GathermateID_to_NodeID[GMID] = NodeID
end


local function mer_EncodeLoc(x, y)
	local x = x *100
	local y = y * 100
	--print("enc")
	if x > 99.99 then
	...
	return math.floor( x * 100  ) * 10000 + math.floor( y * 100  )
end
...
function WoWGatheringNodes:reconvert(DB)

	for map, nodedata in pairs(DB) do
		for nodeID, data in pairs(nodedata) do
			local x,y = floor(nodeID/1000000)/10000, floor(nodeID % 1000000 / 100)/10000
			local encoded = mer_EncodeLoc(x,y)
			local object = GathermateID_to_NodeID[data]
			mergeddatabase[map] = mergeddatabase[map] or {}
			mergeddatabase[map][object] = mergeddatabase[map][object] or ""
			mergeddatabase[map][object] = mergeddatabase[map][object]..encoded..", "
		end
	end
...
function wee()
	for i,db in ipairs(gm2db)do
		--print(i)
	 WoWGatheringNodes:reconvert(db)
	end
